#!/usr/bin/env python3
"""
Nifty 500 Historical Price Pipeline
====================================
Fetches daily OHLCV data for all 500 Nifty 500 constituents from Yahoo Finance
and stores in Nifty500Stock + Nifty500Price tables.
...
Modes:
  full        — fetch 10 years of history (first-time setup, ~3-5 min)
  incremental — fetch only since last stored date (daily cron, ~30 sec)

Usage:
  python3 scripts/fetch_nifty500.py                         # full 10Y
  python3 scripts/fetch_nifty500.py --mode incremental      # update since last date
  python3 scripts/fetch_nifty500.py --mode full --years 5   # custom window
  python3 scripts/fetch_nifty500.py --batch-size 40         # tune batch size

Performance design:
  - 50 tickers per batch  →  10 batches total
  - 4 concurrent workers  →  processes ~200 tickers at once
...
log = logging.getLogger("nifty500")
...
DEFAULT_YEARS      = 10
DEFAULT_BATCH_SIZE = 25    # tickers per yfinance download call (smaller = more reliable)
...
def load_constituents() -> list[dict]:
    """Read nifty500_constituents.csv → list of {symbol, ticker, name, industry, isin}."""
    stocks = []
...
def upsert_stocks(stocks: list[dict]) -> dict[str, str]:
    """
    Upsert all stocks into Nifty500Stock table.
    ...
    with get_conn() as conn:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS "Nifty500Stock" (
                id        TEXT PRIMARY KEY,
        ...
        for s in stocks:
            ...
            cur.execute("""
                INSERT INTO "Nifty500Stock" (id, symbol, ticker, name, industry, isin, "createdAt", "updatedAt")
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS "Nifty500Price" (
                id        TEXT PRIMARY KEY,
                "stockId" TEXT NOT NULL REFERENCES "Nifty500Stock"(id) ON DELETE CASCADE,
                date      DATE NOT NULL,
        ...
        """)
        cur.execute('CREATE INDEX IF NOT EXISTS idx_n500price_stockdate ON "Nifty500Price"("stockId", date)')
        cur.execute('CREATE INDEX IF NOT EXISTS idx_n500price_date ON "Nifty500Price"(date)')

    log.info(f"Upserted {len(ticker_to_id)} stocks into Nifty500Stock")
    return ticker_to_id
...
def _extract_ticker_df(raw: pd.DataFrame, ticker: str) -> pd.DataFrame | None:
    ...
    try:
        ...
        if isinstance(df.columns, pd.MultiIndex):
            # Shouldn't happen with group_by, but flatten if needed
...
def download_batch(
    ...
    try:
        raw = yf.download(
            ...
            progress=False,
            threads=True,
            group_by="ticker",   # explicit grouping — most reliable structure
        )
        if raw.empty:
            return {}
        ...
        if len(tickers) == 1:
            ...
            try:
                ...
                # Some versions nest under ticker even for single-ticker
        ...
        # Sanity: if < 20% returned data, likely rate-limited — treat as failure
...
def bulk_insert_prices(
    ...
    """Bulk-upsert price rows. Returns count of rows processed."""
    rows = []
    ...
    for i in range(0, len(rows), DB_INSERT_CHUNK):
        ...
        execute_values(
            cur,
            """
            INSERT INTO "Nifty500Price" (id, "stockId", date, open, high, low, close, volume)
            VALUES %s
...
def run_pipeline(mode: str = "full", years: int = DEFAULT_YEARS, batch_size: int = DEFAULT_BATCH_SIZE):
    ...
    if mode == "incremental":
        ...
        # Most conservative: use the oldest last_date as start (or 30 days ago fallback)
        ...
        if stored_dates:
            ...
            # No data at all — fall back to full mode
    ...
    with ThreadPoolExecutor(max_workers=DEFAULT_WORKERS) as executor:
        ...
        for i, batch in enumerate(batches):
            # Stagger every batch submission to avoid burst rate-limiting
            if i > 0:
        ...
        for future in as_completed(futures):
            ...
            completed_batches += 1
            tickers_fetched = len(result)
            log.info(f"  ✓ Batch {idx+1} complete: {tickers_fetched} tickers with data")
    ...
    # Insert in sub-batches to keep memory manageable
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Nifty 500 historical price pipeline")
    parser.add_argument(

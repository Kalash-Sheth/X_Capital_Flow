...
function RsiModal({ data, onClose }: { data: RsiResponse; onClose: () => void }) {
  const { niftyBars, rsiBars, extremeEvents, summary, zone, signal, currentRsi, change } = data;
    ...
    <motion.div className="fixed inset-0 z-50 flex flex-col bg-white overflow-hidden"
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden min-h-0">
        ...
        <div className="w-full md:w-72 flex-shrink-0 border-t md:border-t-0 md:border-l border-slate-100 bg-slate-50/40 flex flex-col overflow-y-auto">
          ...
          {summary && (
            <div className="m-4 mb-0 rounded-xl border border-slate-200 bg-white p-4">
              <p className="text-[9px] font-black tracking-wider uppercase text-slate-400 mb-3">
                After RSI &gt; 80 · Nifty Forward Returns
              </p>
              <div className="space-y-3">
                {[
                  { label: "1 Month",   wr: summary.winRate1m,  avg: summary.avgRet1m },
                  { label: "3 Months",  wr: summary.winRate3m,  avg: summary.avgRet3m },
                  { label: "6 Months",  wr: summary.winRate6m,  avg: summary.avgRet6m },
                ].map(({ label, wr, avg }) => {
              ...
              <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-[9px] text-slate-400">{summary.totalEvents} historical events</span>
                <span className="text-[9px] font-bold tabular-nums" style={{ color: "#ef4444" }}>
                  Avg drawdown: {summary.avgMaxDrawdown.toFixed(1)}%
                </span>
              </div>
            </div>
          )}
          ...
          <div className="m-4 rounded-xl p-4 border flex-shrink-0" style={{ background: zone.bgColor, borderColor: `${zone.color}25` }}>
            ...
            <p className="text-xs leading-relaxed text-slate-700">
              {zone.label === "Extreme"
                ? `When Nifty RSI stays above 80 on monthly timeframe, it signals excessive momentum and euphoria. Historically this has led to short-to-medium term corrections as markets become overextended and profit booking begins. ${summary ? `Based on ${summary.totalEvents} past events, 3-month avg return is ${summary.avgRet3m >= 0 ? "+" : ""}${summary.avgRet3m}%.` : ""}`
                : zone.label === "Elevated"
...
function RsiIntelligenceCard() {
  ...
  return (
    <>
      {typeof document !== "undefined" && createPortal(
        <AnimatePresence>
          {modalOpen && <RsiModal data={data} onClose={() => setModalOpen(false)} />}
        </AnimatePresence>,
        document.body
      )}
        ...
        <div className="px-5 py-4 flex items-center gap-5 flex-wrap">
          ...
          {summary && (
            <div className="flex gap-3">
              {[
                { label: "3M avg",  val: summary.avgRet3m },
                { label: "events",  val: summary.totalEvents, isCount: true },
    ...
    <motion.div className="fixed inset-0 z-50 flex flex-col bg-white overflow-hidden"
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden min-h-0">
        ...
        <div className="w-full md:w-72 flex-shrink-0 border-t md:border-t-0 md:border-l border-slate-100 bg-slate-50/40 flex flex-col overflow-y-auto">
          ...
          {ob && (
            <div className="p-4 border-b border-slate-100">
              <p className="text-[9px] font-black tracking-widest uppercase mb-3" style={{ color: "#ef4444" }}>
                After ROC &gt; 80% · Forward Returns
              </p>
          ...
          {os && (
            <div className="p-4 border-b border-slate-100">
              <p className="text-[9px] font-black tracking-widest uppercase mb-3" style={{ color: "#10b981" }}>
                After ROC &lt; -25% · Forward Returns
              </p>
              <div className="space-y-2.5">
...
function RocIntelligenceCard() {
  ...
  const { zone, signal, currentRoc, change, extremeEvents, summary, percentileRank } = data;
  const changeColor = change >= 0 ? "#ef4444" : "#10b981";
  const ob = summary.overbought;
    ...
    <motion.div className="fixed inset-0 z-50 flex flex-col bg-white overflow-hidden"
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden min-h-0">
        ...
        <div className="w-full md:w-72 flex-shrink-0 border-t md:border-t-0 md:border-l border-slate-100 bg-slate-50/40 flex flex-col overflow-y-auto">
          ...
          {/* Thesis scorecard */}
          <div className="p-4 border-b border-slate-100">
            <p className="text-[9px] font-black tracking-widest uppercase text-slate-400 mb-3">Thesis Scorecard</p>
            <div className="space-y-2.5">
              ...
              <WinRateBar wr={summary.winRate24m} avg={summary.avgRet24m} label="24 Months" />
            </div>
